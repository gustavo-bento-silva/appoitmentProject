...
namespace PageNavFrameWork{
	public class PageNav : MonoBehaviour {
		...
		[SerializeField]
		private PageNavSettings settings;
		...
		[HideInInspector]
		public PageController _CurrentPage = null;
		...
		{
			if(Container == null){
				Container = transform.FindChild ("Container").transform;
			}
		...
		public void DisablePage(PageController page){
//			page.gameObject.SetActive ();
		}
		...
		public void PushPageToStack(PagesEnum pageEnum){
		...
		/// </summary>
		/// <param name="PageEnum">Page enum.</param>
		public void PushPageToStack(GameObject pageInstance){
			if(!pageInstance){
				return;
			}
			if(!transitionMutex){
